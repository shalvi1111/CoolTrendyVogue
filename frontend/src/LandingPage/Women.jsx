    ...
    const fetchCateg = async () => {
      try {
        const res = await axios.get(
          "https://cooltrendyvogue.onrender.com/getWomCategory",
        ...
        if (
          ...
          const womenProd = res.data.message.filter(
    ...
    <>
      {category === "Women" ? (
        ...
        <h1 className="fs-4 text-center m-5 p-5">
          {" "}
          "<span style={{ color: "red" }}>Oops!</span> No Women's section?{" "}
          <span style={{ color: "maroon" }}>Impossible! </span>
          <span style={{ color: "purple" }}>Maybe it's just hiding… 👗✨</span>"
        </h1>
      )}
    </>
  );
